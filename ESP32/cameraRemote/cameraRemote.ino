...
// Authentication
UserAuth user_auth(API_KEY, USER_EMAIL, USER_PASS, 3000 /* expire period in seconds (<3600) */);

static unsigned long lastPhotoMillis = 0;
const unsigned long photoInterval = 30000;
...
String getUniquePhotoPath() {
    ...
    if (!getLocalTime(&timeinfo)) {
        now = millis(); // fallback if RTC not set
    } else {
        now = mktime(&timeinfo);
    }
...
void loop(){
    ...
    app.loop();
     ...
     if (app.ready() && (millis() - lastPhotoMillis >= photoInterval)) {
